...
export default function useDuplicateFile() {
    ...
    const duplicateFile = async (editor: Editor, fileNode: FileNodeAttrs) => {
        const newId = uuidv4();
        ...
        try {
            ...
            const file = new File([blob], fileNode.name, { 
                type: fileNode.mimeType,
                lastModified: Date.now()
            });

            let newHref = '';
            if(checkDemo()) {
                newHref = await uploadDemoFile(file);
            } else {
                // 새 ID로 스토리지에 업로드
                newHref = await uploadToStorage(file, `documents/${selectedDocument.id}/files/${newId}`);
            }
