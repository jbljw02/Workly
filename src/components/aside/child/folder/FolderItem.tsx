...
export default function FolderItem({ folder }: FolderItemProps) {
    ...
    const [newFolderTitle, setNewFolderTitle] = useState(folderTitle);

    const completeEdit = async (e: React.KeyboardEvent<HTMLInputElement>) => {
        // 엔터키가 클릭될 시, 작업을 종료하고 폴더명 수정 요청 전송
        if (e.key === 'Enter') {
            const originalTitle = folderTitle; // 변경 전 폴더명 저장

            setFolderTitle(newFolderTitle); // 로컬 상태 즉각 업데이트
            setIsEditing(false);

            // 폴더명 수정 요청
            try {
                await axios.put('/api/folder',
                    { email: user.email, folderID: folder.id, newFolderName: newFolderTitle },
                    {
                        headers: {
                ...
                setFolderTitle(originalTitle); // 요청 실패시 기존 폴더명으로 롤백
            }
        }
        // ESC 키가 클릭될 시, 작업을 종료
        if (e.key === 'Escape') {
